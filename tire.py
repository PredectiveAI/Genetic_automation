...
class GeneticRisk:
    ...
    def execute(self):
        ...
        for self.case_iter,self.case_num in enumerate(cases):
            self.calculate(user_dict[self.case_num])
    ...
    def read_data(self):
        data_f = pd.read_excel('genetic_test-sheet.xlsx')
        param_set_1 = data_f.iloc[1:28,2:]

        param_set_2 = data_f.iloc[28:,2:]
    ...
    def calculate(self,user_dict):
        ...
        f = open('genetic_param.json') 
        param = json.load(f)
        if param['headless'] == "active":
            head_param = True
        else:
            head_param = False
        ...
        workbook = load_workbook(filename="genetic_test-sheet.xlsx")
        sheet = workbook.active

        sheet.cell(row=4, column=4+self.case_iter).value = risk10
        sheet.cell(row=5, column=4+self.case_iter).value = risklt

        workbook.save(filename="genetic_test-sheet.xlsx")
